...
2. **Logs** — written as JSON, tailed by Promtail, pushed to Loki
...
sample-service/
├── README.md            # this file
├── docker-compose.yml   # app + promtail + loki + prometheus
├── app.py               # the service
├── requirements.txt
├── Dockerfile
├── promtail-config.yml  # tails the app's log file → Loki
...
| promtail    | 9080 | tails `app.log`, ships to Loki                          |
| loki        | 3100 | log store                                               |
...
- `promtail-config.yml` is the template for what to ask the platform team to add to `inventories/<env>/group_vars/promtail/main.yml`.
